...
export class InboxHelper {
  ...
  public async checkInbox(sessionId: string): Promise<boolean> {
    try {
      const url = new URL("/api/main/inbox/checkInbox.php", SPTLeaderboard.apiUrl);
      const params = new URLSearchParams(`sessionId=${sessionId}`);

      let response = await fetch(url + params.toString());
      if (!response.ok) {
        this.logger.error(`[SPTLeaderboard] Request to check inbox failed with error ${response.statusText} (${sessionId})`);
        return false;
      }

      let data: ICheckInboxResponse = await response.json() as ICheckInboxResponse;
      if (data.status !== 'success') {
        this.logger.error(`[SPTLeaderboard] Inbox was unsuccessful, with message: ${data.messageText} (${sessionId})`);
        return false;
      }
      ...
      this.mailSendService.sendDirectNpcMessageToPlayer(
        ...
        generatedItems,
        72000);
      this.logger.success(`[SPTLeaderboard] Inbox items received successfully ${sessionId}`);
      return true;
    } catch (error) {
      this.logger.error(`[SPTLeaderboard] Inbox check failed: ${error}`);
      return false;
    }
  }
}
